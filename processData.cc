...
//	  size_t timeBefore = 200;
	  if ((eye.isMSG() != true) && (eye.g_time() >= (trialSet.g_targetOnset() - timeBefore)) && 
	  ...
	  if ((eye.g_time() >= (trialSet.g_targetOnset() - timeBefore)) && 
	    (eye.g_time() <= trialSet.timeIsUp()) && (line.find("SFIX") != string::npos))
